...
describe BinaryGame do
  ...
  describe '#maximum_guesses' do
    ...
    context 'when game minimum and maximum is 100 and 600' do
      ...
      it 'returns 9' do
        max_guess = game_six_hundred.maximum_guesses

        expect(max_guess).to eq(9)
      end
    end
  end
  ...
  describe '#display_binary_search' do
    ...
    context 'when game_over? is false five times' do
      ...
      it 'calls display_turn_order five times' do
        expect(game_display).to receive(:display_turn_order).with(search_display).exactly(5).time
  ...
  describe '#display_turn_order' do
    ...
    let(:bsearch) { instance_double(BinarySearch) }

    before do
      # You'll need to create a few method stubs.
      allow(bsearch).to receive(:make_guess)
      allow(bsearch).to receive(:update_range)
      allow(new_game).to receive(:display_guess)
      allow(bsearch).to receive(:game_over?)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      expect { new_game.display_turn_order(bsearch) }.to change { new_game.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(bsearch).to receive(:make_guess).once

      new_game.display_turn_order(bsearch)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(bsearch).to receive(:update_range).once
